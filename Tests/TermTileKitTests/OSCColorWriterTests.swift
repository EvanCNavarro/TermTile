...
struct OSCColorWriterTests {
    ...
    func writesSequence() async {
        ...
        #expect(writes.first?.bytes == "\u{1B}]1337;SetColors=bg=143C22\u{07}")
    }
    ...
    func statesMapToColours() async {
        ...
        #expect(writes.contains { $0.bytes.contains("4A320F") })
        #expect(writes.contains { $0.bytes.contains("111417") })
