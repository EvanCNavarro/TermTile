...
struct SettingsStoreTests {
    ...
    // #36 — bring-to-front is opt-in: absent → false (upgrade-safe; Rearrange keeps today's
    // no-focus behavior unless the user explicitly enables it); round-trips true.
    @Test("bringToFrontOnRearrange defaults false; round-trips true")
    func bringToFrontOnRearrangePersists() {
        #expect(AppSettings.defaults.bringToFrontOnRearrange == false)
        let suite = "dev.ecn.apps.termtile.tests.bringfront"
        UserDefaults(suiteName: suite)?.removePersistentDomain(forName: suite)
        defer { UserDefaults(suiteName: suite)?.removePersistentDomain(forName: suite) }
