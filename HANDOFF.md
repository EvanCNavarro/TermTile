...
| Latest release | **v0.2.1** (2026-07-16), build 123 |
| Public signing | Developer ID Application: Evan Navarro (`XG9SBNWNXT`) |
| Notarization | Pending Apple Notary processing; see `docs/NOTARIZATION.md` |
...
2. **Poll notarization, do not resubmit.** Use `docs/NOTARIZATION.md` and `scripts/notary-status.sh`
   to check the existing fixed TermTile and `NotaryProbe` jobs. If Apple returns `Accepted`, staple
   and assess the exact accepted artifact, then wire notarization into release CI for the follow-up
   release.
...
- **Released & live:** v0.2.1 - menu-bar window-tiler: pick a terminal (iTerm2/WezTerm), press
  **Rearrange now**, and windows snap into even columns of two. `v0.2.1` is Developer ID signed to
  stabilize macOS TCC grants across updates, but is not notarized yet.
...
- **Notarized follow-up release** - gated on Apple Notary returning an accepted ticket for the existing
  fixed jobs or a future final artifact. See `docs/NOTARIZATION.md`.
